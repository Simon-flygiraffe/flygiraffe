...
def get_nic(ssh):
    stdin, stdout, stderr = ssh.exec_command("lshw -c net -businfo ; ip link show | grep 'state UP'")
...
def get_time(ssh):
    stdin, stdout, stderr = ssh.exec_command(" date ; ntpstat ; timedatectl status | grep 'enabled' ")
...
def get_ips(ssh):
    stdin, stdout, stderr = ssh.exec_command("ip -4 addr | grep -v '127.0.0.1' | grep 'inet' ")
    cmd6 = stdout.readlines()
    cmd6 = "".join(cmd6)
    ips = cmd6
    return ips

def get_oss(ssh):
    stdin, stdout, stderr = ssh.exec_command(" hostnamectl | grep 'Kernel' ; hostnamectl | grep 'Operating System' ; hostnamectl | grep 'Architecture' ")
    cmd7 = stdout.readlines()
    cmd7 = "".join(cmd7)
    oss = cmd7
    return oss
    ...
    with open("nodes_info.txt", 'w') as f:
        for host in hosts_file:
            ...
            try:
                ...
                f.write(lineopen + "\nSYSTEM INFO FOR HOST:" + lineclose + "    ---->>>     " + str(get_hostname(ssh)) +
                        lineopen + "\nOS & KERNEL DETAILS:\n" + lineclose + str(get_oss(ssh)) +
                        lineopen + "\nALL IP-V4 ADDRESSES:\n" + lineclose + str(get_ips(ssh)) + "\n\n" +
                        lineopen + "SYSTEM TIME & NTP STATUS: \n" + lineclose + str(get_time(ssh)) +
                        "\n\n" + lineopen + "CPU MODEL:\n" + lineclose + str(get_cpu(ssh)) + "\n\n" +
                        lineopen + "NUM OF CPU'S IN THE SERVER: \n" + lineclose + str(get_cpu_count(ssh)) +
                        "\n\n" + lineopen + "LIST EXISTING NETWORK CARDS & WHICH NIC IS UP:\n" + lineclose + str(get_nic(ssh)) +
                        "\n\n" + lineopen + "TOTAL NUM OF NVME DEVICES:\n" + lineclose + str(get_nvme_count(ssh)) +
                        "\n\n" "***END OF REPORT FOR THIS HOST*** " + ' > '*15 + " NEXT HOST..." + "\n\n ")
