...
import argparse  # arguments parsing library
import time
import os
...
def main():
    ...
    print("\n --> System Report file will be created under: " + str(os.getcwd()) + "\n" + '.' * 50 + "\n")
    time.sleep(3)
    ...
    with open("nodes_sys_info.txt", 'w') as f:
        for host in hosts_file:
            ...
            try:
                ...
                try: # catch issue if happen
                    # connect the client to the server host:port with the credentials
                    ssh.connect(host, port, username, password)
                except: # Throw exception if ssh connection isn't established
                    print("!! **ERROR** !!: Could not establish connection with host: " + str(host) + "\n - Please check password and ssh connection..")
                    raise # Exit host is ssh not established
                ...
                print("something is wrong wile running commands on host:  " + str(host) + "\n")
